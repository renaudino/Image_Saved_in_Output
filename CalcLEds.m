...
global Ninterac DSFlag wedges GDSFlag DSN PDS FDS Area vnfaces emett ...
       recept lambda OutputDataLDS TransmitType Lt ReceiveType Lr Mtrun Nbrimg;
...
for i = 1:Ninterac
    ...
    while (right(1) ~= 0)  % if not empty, then continue
        ...
        if ((bl ~= 0) && (DSFlag(bl) == 1)) || ((bl == 0) && (GDSFlag == 1))
...
%           pointR(:,1) = Breakthrough(back(1,2:3),back(1,4:6),cntpt);
            DSN = 0;
...
%                   pointV(:,1) = Breakthrough(rback(1,2:3),rback(1,4:6),extrem);
                    glob = 1;
                    track(m,:) = pointV;
...
%                                   RefFieldlx(:,1) = RefField(fb1,0,e,PDS(j,:),track(1,:),GroundN,Ein2); 
...
%                                   RefFieldlx(:,1) = RefField(fb1,trackface(1,1),e,PDS(j,:),track(1,:),vnfaces(trackface(1,1),trackface(1,2),:),Ein2);
                                ...
                                if (trackface(i-l+1,1) == 0)
                                    ...
                                    RefFieldlx(:,1) = RefField(fb1,0,Re3,Re4,Re5,GroundN,Ein2);
%                                   RefFieldlx(:,1) = RefField(fb1,0,e,track(i-l,:),track(i-l+1,:),GroundN,Ein2);
                                    ...
                                    RefFieldlx(:,1) = RefField(fb1,Re2,Re3,Re4,Re5,vn_temp,Ein2);
%                                   RefFieldlx(:,1) = RefField(fb1,trackface(i-l+1,1),e,track(i-l,:),track(i-l+1,:),vnfaces(trackface(i-l+1,1),trackface(i-l+1,2),:),Ein2);
...
%                                   [Eds(:,1),dr] = dsfield(Ein2,track(1,:),recept,FDS(j,:),PDS(j,:),Area(j));
                                    flagds = 0;
                                    dr = dr+dt+dt1+dr1;
                                    ...
                                    if (flagds ~= 1)
                                        ...
                                        OutputDataLDS(c,:) = [emett(1),emett(2),emett(3),recept(1),recept(2),recept(3),track(i,1),track(i,2),track(i,3),PDS(j,1),...
                                            PDS(j,2),PDS(j,3),dr,real(Eout),imag(Eout),3,1];
                                    end
                                end
                        end
                    end
