function CalcEDiff_Image_Olivier ()

% Diffraction field component only consider the 1st order diffraction
...
% global DiffASLN DiffFSLN DiffNFSLN DiffPSLN EFMAX emett eta K lambda Lr Lt Nblock ...
%        OutputDataDiff ReceiveType recept TransmitType vnfaces wedges;
global DiffASLN DiffFSLN DiffNFSLN DiffPSLN EFMAX emett lambda Lr Lt ...
       OutputDataDiff ReceiveType recept TransmitType vnfaces;
...
for i = 1:NdiffPath
    ...
    if ((me.mask ~= 1) && (mr.mask ~= 1))
        ...
        if (flag ~= 1)
            Ediff(:,1)            = DiffField(DiffFSLN(i,1),emett,DiffPSLN(i,:),recept, ...
                                    vnfaces(DiffFSLN(i,1),DiffFSLN(i,2),:),vnfaces(DiffNFSLN(i,1),DiffNFSLN(i,2),:), ...
                                    DiffASLN(i,:),Einc);
            if (mr.mask == 2)
                for l = 1:mr.npb
            ...
            if (flag ~= 1)
                lambda1      = lambda*(dt1+norm(ep))/(dt1+norm(ep)+df);
                ...
                OutputDataDiff(c,:) = [emett(1),emett(2),emett(3),recept(1),recept(2),recept(3),df,real(Eout),imag(Eout),2,1,...
                                       DiffPSLN(i,1),DiffPSLN(i,2),DiffPSLN(i,3),zeros(1,4*3),zeros(1,5*3),zeros(1,5*4)];
            end
        end
    end
end