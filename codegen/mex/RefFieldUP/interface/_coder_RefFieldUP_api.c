/*
 * _coder_RefFieldUP_api.c
 *
 * Code generation for function '_coder_RefFieldUP_api'
 *
 */
  ...
  u[3])
{
  ...
  y = NULL;
  m3 = emlrtCreateNumericArray(1, iv2, mxDOUBLE_CLASS, mxCOMPLEX);
  ...
  emlrtMsgIdentifier *msgId, emxArray_creal_T *ret)
{
  ...
  i6 = ret->size[0];
  ret->size[0] = iv4[0];
  emxEnsureCapacity(sp, (emxArray__common *)ret, i6, sizeof(creal_T),
                    (emlrtRTEInfo *)NULL);
  emlrtImportArrayR2015b(sp, src, ret->data, 8, true);
  emlrtDestroyArray(&src);
}

void RefFieldUP_api(const mxArray * const prhs[7], const mxArray *plhs[3])
{
  real_T pblo;
  real_T blo;
  real_T (*E_re)[3];
  ...
  RefFieldUP(&st, pblo, blo, *E_re, *R_re, *qr_re, *n_re, Ein, Eout, &Rt, &Rh);
...
/* End of code generation (_coder_RefFieldUP_api.c) */
