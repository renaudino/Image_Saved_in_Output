function [Eout] = CalcELOS_Image_Olivier ()
...
global EFMAX emett TransmitType lambda Lr Lt OutputDataLos ReceiveType recept;

global Nblock eta K vnfaces wedges;
...
if (bk ~= 1)
    ...
    if strcmp(methM,'Masking')
        ...
        if (mmask ~= 2); los = struct('mask',mmask,'npb',los.npb); end
    else
        los = Masking_Olivier_global(Temp1,Temp2);
        if isempty(los.pflag)
            los = struct('mask',los.mask,'npb',los.npb);
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if (los.mask ~= 1)
        ...
        if (los.mask == 2)
            for i = 1:los.npb
                if (los.pflag(i,1) == 1) || (los.pflag(i,2) == 4)
                    fprintf('   Error: LOS - P1 and P2 cannot be the reflection points! \n');
                    exit;
                end
        ...
        % '5' -> maximum number of reflections we considered in ray traing tool
